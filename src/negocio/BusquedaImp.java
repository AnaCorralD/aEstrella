...
public class BusquedaImp implements Busqueda {
	...
	private ArrayList<Nodo> encontrarAdyacentes(TAlgoritmo tAlgoritmo, Nodo actual, Nodo nodoFinal){
		...
		//Arriba
		if(x > 0) {
			if (!tAlgoritmo.getModo() || (tAlgoritmo.getModo() && tAlgoritmo.getAltura() >= tAlgoritmo.getMatriz()[x - 1][y].getAltura())) {
		...
		//Izquierda
		if(y > 0) {
			if (!tAlgoritmo.getModo() || (tAlgoritmo.getModo() && tAlgoritmo.getAltura() >= tAlgoritmo.getMatriz()[x][y-1].getAltura())) {
		...
		if(diagonalArribaIzquierda &&  tAlgoritmo.getMatriz()[x - 1][y - 1].getTipo() != Casillas.BLOQUEADO) {
			if (!tAlgoritmo.getModo() || (tAlgoritmo.getModo() && tAlgoritmo.getAltura() >= tAlgoritmo.getMatriz()[x - 1][y - 1].getAltura())) {
	...
	private void addNodoAListaAbierta(ArrayList<Nodo> listaAbierta, Nodo actual) {
		int i = 0;
				...
				for(Nodo i: adyacentes) {
					if(!buscarListaCerrada(listaCerrada, i)) { //Si no está en la cerrada
						...
						if(n == null) { //Si no está en la lista de abiertas lo metemos
							addNodoAListaAbierta(listaAbierta,i);
						}
							...
							if(i.getCosteTotal() < n.getCosteTotal()) {//Reevaluo el coste
								...
								addNodoAListaAbierta(listaAbierta, i); //Añado el nodo que tiene mejor coste a la lista
							}
						}
					}
				}
